...
mean_tready: out STD_LOGIC_VECTOR(NUM_STREAMS-1 downto 0);
...
signal mean_tdata_streams: streamtype := (others => (others => '0'));
signal mean_tvalid_streams: STD_LOGIC_VECTOR(NUM_STREAMS-1 downto 0) := (others => '0');
signal mean_tready_streams: STD_LOGIC_VECTOR(NUM_STREAMS-1 downto 0) := (others => '0');
...
signal fifo_mean_m_tvalid: STD_LOGIC_VECTOR(NUM_STREAMS-1 downto 0) := (others => '0');
signal fifo_mean_m_tready: STD_LOGIC_VECTOR(NUM_STREAMS-1 downto 0) := (others => '0');
signal fifo_mean_m_tdata: streamtype := (others => (others => '0'));
signal fifo_mean_select : integer range 0 to NUM_STREAMS-1 := 0;
...
-- direct mean inputs to correct fifo
process(aclk)
begin
if rising_edge(aclk) then
    if aresetn = '0' then
        mean_tvalid_streams <= (others => '0');
        mean_tdata_streams <= (others => (others => '0'));
    else
        mean_tvalid_streams <= (others => '0');
        mean_tdata_streams <= (others => (others => '0'));
        
        mean_tvalid_streams(conv_integer(mean_id)) <= mean_tvalid;
        mean_tdata_streams(conv_integer(mean_id)) <= mean_tdata;
    end if;
end if;
end process;
...
begin
    if rising_edge(aclk) then
        if aresetn = '0' then
            ...
            if fifo_mean_m_tvalid(fifo_mean_select) = '1' and mean_fifo_tready = '1' then
                fifo_mean_ready_pulse <= (others => '0');
                fifo_mean_ready_pulse(fifo_mean_select) <= '1';
                
                if fifo_mean_select = NUM_STREAMS-1 then
                    fifo_mean_select <= 0;
                else
                    fifo_mean_select <= fifo_mean_select + 1;
                end if;
            else
                fifo_mean_ready_pulse <= (others => '0');
            end if;
...
fifo_mean_m_tready <= fifo_mean_ready_pulse;

x_fifo_tvalid <= fifo_x_m_tvalid(fifo_x_select);
x_fifo_tdata <= fifo_x_m_tdata(fifo_x_select);

mean_fifo_tvalid <= fifo_mean_m_tvalid(fifo_mean_select);
mean_fifo_tdata <= fifo_mean_m_tdata(fifo_mean_select);
...
fifo_instances_mean: for i in 0 to NUM_STREAMS-1
generate fifo_mean_inst_i: fifo32x16_buffering
    port map ( s_axis_aresetn => aresetn,
    s_axis_aclk => aclk,

    s_axis_tvalid => mean_tvalid_streams(i),
    s_axis_tready => mean_tready_streams(i),
    s_axis_tdata => mean_tdata_streams(i),
            
    m_axis_tvalid => fifo_mean_m_tvalid(i),
    m_axis_tready => fifo_mean_m_tready(i),
    m_axis_tdata => fifo_mean_m_tdata(i));
end generate;
