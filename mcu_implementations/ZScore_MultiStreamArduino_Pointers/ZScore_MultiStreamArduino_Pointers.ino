...
bool zScore(StreamData &stream, float t) {
  ...
  // Compute mean & standard deviation efficiently
...
void loop() {
  ...
  float t = random(1, 5); // Random temperature
  if (sampleCount > (N * STREAM_COUNT) && random(0, 100) < 10) {  
    t = random(80, 100); // 10% chance of anomaly
  }
  ...
  currentStream = (currentStream + 1) % STREAM_COUNT; // Cycle through streams
  delay(500);
}
